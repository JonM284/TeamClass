...
public class Do_Damage_Support : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.gameObject.tag == "Player" && is_Spike)
        {
            ...
            {
                collision.GetComponent<BasicPlayerScript>().GetHit(attack_Damage, angle, 0 ,0.2f, 60f, 0.5f, false, 0.1f, 0.3f, 0.2f);
            }else if (transform.position.x > collision.transform.position.x)
            {
                collision.GetComponent<BasicPlayerScript>().GetHit(attack_Damage, angle, 0 ,0.2f, 60f, 0.5f, true, 0.1f, 0.3f, 0.2f);
            }
