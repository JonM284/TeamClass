...
public class SideCannon_projectile : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if (other.gameObject.tag == "Player") {
            if(move_Right)
            other.gameObject.GetComponent<BasicPlayerScript>().GetHit(150f, 45, 120, 0.2f, 100, .2f, false, 0.1f, 0.3f, 0.2f);

            if (!move_Right)
                other.gameObject.GetComponent<BasicPlayerScript>().GetHit(150f, 45, 120, 0.2f, 100, .2f, true, 0.1f, 0.3f, 0.2f);
        }
    }
